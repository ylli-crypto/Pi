...
function normalizeProviderInput(value: unknown): SearchProvider | undefined {
	...
	const valid: SearchProvider[] = ["auto", "openai", "brave", "tavily", "exa", "perplexity", "gemini"];
...
export default function (pi: ExtensionAPI) {
	...
	if (initConfig.webSearch?.enabled !== false) pi.registerTool({
		...
		label: "Web Search",
		description:
			`Search the web using OpenAI, Brave, Tavily, Exa, Perplexity, or Gemini. Returns an AI-synthesized answer with source citations. OpenAI web_search uses a Codex subscription or OpenAI API key. For comprehensive research, prefer queries (plural) with 2-4 varied angles over a single query — each query gets its own synthesized answer, so varying phrasing and scope gives much broader coverage. When includeContent is true, full page content is fetched in the background. Searches auto-open the interactive browser curator and stream results live; set workflow to "none" to skip curation or "auto-summary" for a model-generated summary without the browser curator. Provider auto-selects: OpenAI when suitable and available, then Exa, Brave, Tavily, Perplexity, Gemini API, then Gemini Web.`,
		promptSnippet:
		...
		parameters: Type.Object({
			...
			provider: Type.Optional(
				StringEnum(["auto", "openai", "brave", "tavily", "exa", "perplexity", "gemini"], { description: "Search provider (default: auto)" }),
			),
