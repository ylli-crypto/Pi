...
function getProfileReviewerModel(profile: { subagents?: { agentOverrides?: Record<string, { model?: string }> } }): string | undefined {
	const model = profile.subagents?.agentOverrides?.reviewer?.model;
...
export function registerSlashCommands(
	...
	pi.registerCommand("chain", {
		description: "Run agents in sequence: /chain scout \"task\" -> reviewer [--bg] [--fork]",
		...
		handler: async (args, ctx) => {
			...
			try {
				await withSlashStatus(ctx, `Loading profile ${parsed.value}…`, async () => {
					...
					const reviewerModel = getProfileReviewerModel(profile);
					...
					if (reviewerModel && typeof pi.setModel === "function" && typeof ctx.modelRegistry?.find === "function" && typeof ctx.modelRegistry?.getAvailable === "function") {
						const shouldSwitch = await ctx.ui.confirm(
							"",
							`Profile loaded. Also switch this session to the profile reviewer model?\n\n${reviewerModel}`,
						);
						if (shouldSwitch) {
							const modelInfo = findModelInfo(reviewerModel, ctx.modelRegistry.getAvailable().map(toModelInfo));
							const model = modelInfo ? ctx.modelRegistry.find(modelInfo.provider, modelInfo.id) : undefined;
							if (!modelInfo || !model) {
								lines.push(`Could not switch current session model: '${reviewerModel}' is not available in the current model registry.`);
							} else {
								const success = await pi.setModel(model);
								if (success) lines.push(`Current session model switched to: ${modelInfo.fullId}`);
								else lines.push(`Could not switch current session model to '${reviewerModel}': no API key or provider access is available.`);
							}
						}
					} else if (reviewerModel) {
						lines.push(`Profile reviewer model: ${reviewerModel}`);
					}
